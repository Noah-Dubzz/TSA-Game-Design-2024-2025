...
with (objUnit) {
    if (global.currentturn == 3 && owner == objP3 && type == "Gatherer") {
        hasGatherer = true;
...
with (objUnit) {
    if (global.currentturn == 3 && target == objP3) {
        underAttack = true;
...
if (!instance_exists(objP1) && !instance_exists(objP2) && !instance_exists(objP4)) {
    // All three players (P1, P2, P3) are gone
    global.winner = 3;
	room_goto(rmWin);
} 
// Check if it's Player 3's turn
if (global.currentturn == 3) {
    ...
    global.colliding = place_meeting(x, y, objP3UnitGenerator);
    ...
    // If it's not Player 3's turn, keep the sprite idle
