...
if (global.currentplayer == objP2 && place_meeting(x, y, objP2) && keyboard_check_pressed(ord("E"))) {
    // Calculate cost based on current upgrades.
    var cost = upgrade_cost * (1 + upgrades);
    if (objP2.resources >= cost) {
        objP2.resources -= cost;
        upgrades += 1;
		hp = 100;
...
if (active) {
    ...
    if (!instance_exists(current_target) || point_distance(x, y, current_target.x, current_target.y) > detection_range) {
        ...
        for (var i = 0; i < count; i++) {
            ...
            if (unit.owner != objP2) { // Only target enemy units
